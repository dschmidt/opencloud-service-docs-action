...
site:
  ...
  docs_dir: .github/docs/pages           # default; overlay dir for extra/overriding pages, see below
...
`site.docs_dir` (default `.github/docs/pages/`) is copied **over** the
template's `docs/` tree before the site builds:

- A file at the same relative path **replaces** the template page —
  `pages/intro.md` replaces the default overview (keep its front matter:
...
Don't put the overlay at `.github/docs/docs/` — that path is swept after
every build (an upstream generator template writes a stray artifact there).
...
13. Copy `<site>/build/` → `$DOCS_OUTPUT` (the repo's Makefile typically overrides to `docs/generated/`). The build stays in place at `.cache/site/build/` so `pnpm run serve` in the cache dir just works.
