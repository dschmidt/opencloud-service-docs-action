...
# Docs overlay: pages the service repo layers over the template's docs/.
# A file at the same relative path replaces the template page (intro.md
# replaces the default overview); anything else becomes an additional page,
# picked up by the autogenerated sidebar.
if [ -d "$DOCS_OVERLAY_DIR" ]; then
  echo "==> overlaying docs pages from $DOCS_OVERLAY_DIR"
  cp -R "$DOCS_OVERLAY_DIR/." "$SITE_DIR/docs/"
fi

# Announcement bar: site-wide, non-closable notice on every page. The text
# is JSON-encoded via yq so quotes/newlines survive into the TS config;
# inline HTML is allowed (Docusaurus renders the content as HTML).
if [ -n "$SITE_ANNOUNCEMENT" ]; then
  ANNOUNCEMENT_JSON="$(yq -o=json '.site.announcement' "$CONFIG_PATH")"
  SITE_ANNOUNCEMENT_BLOCK="announcementBar: { id: 'service-docs-announcement', content: ${ANNOUNCEMENT_JSON}, isCloseable: false },"
else
  SITE_ANNOUNCEMENT_BLOCK=''
fi

# Render docusaurus.config.ts from template.
export SITE_TITLE SITE_TAGLINE SITE_ORG SITE_REPO SITE_URL SITE_BASE_URL SITE_GITHUB_URL SITE_ANNOUNCEMENT_BLOCK
envsubst '${SITE_TITLE} ${SITE_TAGLINE} ${SITE_ORG} ${SITE_REPO} ${SITE_URL} ${SITE_BASE_URL} ${SITE_GITHUB_URL} ${SITE_ANNOUNCEMENT_BLOCK}' \
  < "$SITE_DIR/docusaurus.config.ts.tmpl" \
  > "$SITE_DIR/docusaurus.config.ts"
rm "$SITE_DIR/docusaurus.config.ts.tmpl"

# Substitute service name in template wrappers and overlay pages alike.
find "$SITE_DIR/docs" \( -name '*.mdx' -o -name '*.md' \) -exec sed -i.bak "s/@@SERVICE@@/$SERVICE_NAME/g" {} +
